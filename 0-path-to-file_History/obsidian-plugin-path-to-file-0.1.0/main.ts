...
export default class PathToUriPlugin extends Plugin {
	...
	private async openClipboardPath() {
		...
		if (isInCurrentVault) {
			// 策略 1：直接在当前 vault 中打开文件
			const relativePath = normalizedPath.slice(normalizedVaultBase.length).replace(/^\/+/, '');
			await this.openFileInCurrentVault(relativePath, normalizedPath);
		} else {
			// 策略 2：转换为 obsidian:// URI，通过系统协议打开（跨 vault）
	 ...
	 * 在当前 vault 中直接打开文件
	 */
	private async openFileInCurrentVault(relativePath: string, originalPath: string) {
		...
		if (file instanceof TFile) {
			...
			// 文件在 vault 目录下但不在 Obsidian 索引中（可能未被 Obsidian 识别）
			// 尝试用 openLinkText 触发打开
			try {
				...
				new Notice('当前 vault 中找不到该文件：' + originalPath.slice(0, 50));
			}
		}
	}
