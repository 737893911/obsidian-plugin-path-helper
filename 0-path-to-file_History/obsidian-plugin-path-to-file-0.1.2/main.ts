...
export default class PathToUriPlugin extends Plugin {
	 ...
	 * 如果文件夹下有一个和文件夹同名的笔记（如 folder/folder.md），就打开它；否则提示找不到。
	 */
	private async openFolderNote(folder: TFolder, relativePath: string) {
		...
		if (folderNote instanceof TFile) {
			...
			new Notice('找不到同名文件夹笔记：' + folderNotePath);
		}
	}
...
class PathToUriSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(containerEl)
			...
			.setDesc('打开文件夹时，尝试打开同名笔记（如 folder/folder.md）；找不到则提示。关闭时仅在文件管理器中定位文件夹。')
			.addToggle((toggle) =>
				toggle
