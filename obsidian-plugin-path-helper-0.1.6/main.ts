...
export default class PathHelperPlugin extends Plugin {
	...
	async onload() {
		...
		// 接力方案：注册自定义 URI 处理器，接收来自其他 vault 的文件夹定位请求
		// 当其他 vault 发送 obsidian://path-helper-reveal?vault=本仓库&folder=相对路径 时，
		// 本处理器被触发，在文件管理器中定位该文件夹
		this.registerObsidianProtocolHandler('path-helper-reveal', (params) => {
	...
	/**
	 * 跨 vault 场景：读取 obsidian.json 中的已注册 vault 列表，匹配后构建 URI 打开
	 *
	 * - 文件：使用标准 obsidian://open?vault=X&file=... 打开
	 * - 文件夹：使用自定义接力 URI obsidian://path-helper-reveal?vault=X&folder=...
	 *   要求目标 vault 也安装了本插件（其 onload 时注册的处理器会定位该文件夹）
	 */
	private openViaRegisteredVault(rawPath: string) {
		...
		if (isDirectory) {
			// 文件夹：使用接力 URI，要求目标 vault 也安装了本插件
			uri = `obsidian://path-helper-reveal?vault=${encodeURIComponent(result.vault)}&folder=${encodeURIComponent(result.file)}`;
			noticeText = '正在定位文件夹：' + result.file + '\n（目标仓库需安装 Path Helper 插件）';
		} else {
			// 文件：使用标准 open URI
			uri = result.uri;
			noticeText = '正在打开：' + result.file;
		}
