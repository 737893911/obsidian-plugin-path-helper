...
export default class PathHelperPlugin extends Plugin {
	...
	openSyncVaultsModal() {
		...
		// 排除当前仓库
		const otherVaults = allVaults.filter((v) => normalize(v.path) !== currentNormalized);
		...
		new SyncToAllVaultsModal(this.app, otherVaults, this.getCurrentVersion(), (selected) => {
	 ...
	 * - 关闭安全模式：app.json 的 restrictedMode 设为 false
	 */
	private syncToVaults(targetVaults: { path: string; configDir: string }[]) {
		...
		for (const vault of targetVaults) {
			...
			try {
				...
				const targetPluginDir = path.join(obsidianDir, 'plugins', pluginId);

				// 版本比对
				let targetVersion = '';
				const targetManifestPath = path.join(targetPluginDir, 'manifest.json');
				...
				// 关闭安全模式
				this.disableRestrictedMode(obsidianDir);
	...
	/**
	 * 关闭指定仓库的安全模式（app.json 的 restrictedMode 设为 false）
	 */
	private disableRestrictedMode(obsidianDir: string) {
		const fs = require('fs');
		const path = require('path');
		const filePath = path.join(obsidianDir, 'app.json');

		let appConfig: any = {};
		if (fs.existsSync(filePath)) {
			try {
				appConfig = JSON.parse(fs.readFileSync(filePath, 'utf-8'));
			} catch {
				appConfig = {};
			}
		}

		if (appConfig.restrictedMode !== false || appConfig.safeMode !== false) {
			appConfig.restrictedMode = false;
			appConfig.safeMode = false;
			fs.writeFileSync(filePath, JSON.stringify(appConfig, null, '\t'), 'utf-8');
		}
	}
}
...
class PathHelperSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(containerEl)
			...
			.setDesc('将当前版本的 Path Helper 安装/更新到其他已注册的 Obsidian 仓库（会自动启用插件并关闭安全模式）。建议先关闭其他仓库的 Obsidian 窗口再执行。')
			.addButton((button) =>
				button
...
class SyncToAllVaultsModal extends Modal {
	...
	constructor(app: App, vaults: VaultInfo[], currentVersion: string, onConfirm: (selected: VaultInfo[]) => void) {
		super(app);
		this.vaults = vaults;
	...
	private getVaultPluginInfo(vault: VaultInfo): {
		...
		const manifestPath = path.join(obsidianDir, 'plugins', 'path-helper', 'manifest.json');
		...
		try {
			...
			if (fs.existsSync(cpPath)) {
				...
				if (Array.isArray(list) && list.includes('path-helper')) {
					enabled = true;
				}
			}
		} catch {
		...
		try {
			...
			if (fs.existsSync(appJsonPath)) {
				...
				safeMode = appConfig.restrictedMode !== false || appConfig.safeMode !== false;
			}
		} catch {
			// ignore
		}

		return { installed, version, isLatest, enabled, safeMode };
	}
	...
	onOpen() {
		...
		contentEl.createEl('p', {
			text: '勾选需要同步的仓库。同步会安装/更新插件、启用插件并关闭安全模式。建议先关闭这些仓库的 Obsidian 窗口。',
		});
		...
		this.vaults.forEach((vault) => {
			...
			if (info.installed) {
				...
				statusParts.push(info.safeMode ? '安全模式开' : '安全模式关');
			}
