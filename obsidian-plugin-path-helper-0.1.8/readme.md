...
- 🔄 **一键同步到所有仓库**：在设置面板一键将本插件安装/更新到其他已注册的 Obsidian 仓库，自动启用插件并关闭安全模式
...
1. 下载或克隆本项目
2. 将 `obsidian-plugin-path-helper-0.1.8` 文件夹复制到你的 vault 插件目录：
   ```
...
当你在多个 Obsidian 仓库中使用本插件时，每次升级都要逐个仓库复制文件、启用插件、关闭安全模式，非常繁琐。v0.1.8 提供了一键同步功能。
...
- **关闭安全模式**：将目标仓库 `app.json` 中的 `restrictedMode` 和 `safeMode` 设为 `false`

### 仓库状态显示

弹窗中每个仓库会显示：
- **未安装** / **v0.1.x（可更新至 v0.1.8）** / **v0.1.8（最新）**
- **已启用** / **未启用**
- **安全模式开** / **安全模式关**
...
```
obsidian-plugin-path-helper-0.1.8/
...
- 仓库选择弹窗：
  - 列出所有其他已注册仓库，显示安装状态（未安装 / 版本号 / 可更新 / 最新）、是否启用、安全模式状态
  - 默认全部未勾选，支持全选/取消全选
- 同步行为：
  ...
  - **关闭安全模式**：`app.json` 的 `restrictedMode` 和 `safeMode` 设为 `false`
